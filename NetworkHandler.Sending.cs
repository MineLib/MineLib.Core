...
namespace MineLib.Network
{
    ...
    public sealed partial class NetworkHandler
    {
        ...
        {
            if (_customModule)
                return new CustomModuleAsyncResult<Boolean>(ModuleConnectToServer, asyncCallback, state);
        ...
        {
            if (_customModule)
				return new CustomModuleAsyncResult<Boolean>(() => ModuleKeepAlive(value), asyncCallback, state);
        ...
        {
            if (_customModule)
                return new CustomModuleAsyncResult<Boolean>(ModuleSendClientInfo, asyncCallback, state);
        ...
        {
            if (_customModule)
                return new CustomModuleAsyncResult<Boolean>(ModuleRespawn, asyncCallback, state);
        ...
        {
            if (_customModule)
                return new CustomModuleAsyncResult<Boolean>(() => ModulePlayerMoved(data), asyncCallback, state);
        ...
        {
            if (_customModule)
                return new CustomModuleAsyncResult<Boolean>(() => ModulePlayerSetRemoveBlock(data), asyncCallback, state);
        ...
        {
            if (_customModule)
                return new CustomModuleAsyncResult<Boolean>(() => ModuleSendMessage(message), asyncCallback, state);
        ...
        {
            if (_customModule)
                return new CustomModuleAsyncResult<Boolean>(() => ModulePlayerHeldItem(slot), asyncCallback, state);
